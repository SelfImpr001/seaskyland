 ...
 * @version 1.0
 */
public class KnowledgeContentCjCalculate implements ICalcluator {
	...
	public void calculate(CalculateTask event,CalculateResult cr,SubjectScoreContainer ssc) {
		...
		List<ItemGroup> itemGroups = ItemGroup.createItemGroupsWithItemAttr(
				"knowledgeContent", atp.getItems(),event.getContext().isHasChoice());
	...
	private TargetResultContainer getTargetResultContainer(CalculateResult cr,
			ItemGroup itemGroup) {
		return cr.getKnowledgeContentTargetResult(itemGroup);
	}
