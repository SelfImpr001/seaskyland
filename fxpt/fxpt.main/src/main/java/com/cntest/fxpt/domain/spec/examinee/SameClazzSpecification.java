 ...
 * @version 1.0
 **/
public class SameClazzSpecification extends AbstractSpecification<ExamStudent> {
	private static String sql = "SELECT COUNT(id) FROM kn_studentbase WHERE  classCode= ? ";

	@Override
	public boolean isInSatified(ExamStudent t) {
		JdbcTemplate jdbcTemplate = SpringContext.getBean("jdbcTemplate");
		int i = jdbcTemplate.queryForInt(sql, t.getClazz().getCode());
		return i > 0;
	}
