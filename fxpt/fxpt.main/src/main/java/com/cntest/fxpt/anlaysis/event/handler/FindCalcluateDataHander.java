 ...
 * @version 1.0
 */
public class FindCalcluateDataHander implements EventHandler<CalculateTask>,
		WorkHandler<CalculateTask> {
	private static final Logger log = LoggerFactory
			.getLogger(FindCalcluateDataHander.class);

	@Override
	public void onEvent(CalculateTask event) throws Exception {
		try {
			IExamContext context = event.getContext();
			StudentCjContainer scjc = context
					.getStudentCjContainer(new OrgFilter(event.getObj()));
