...
import com.cntest.fxpt.anlaysis.calculate.impl.XinjiangCombinationSubjectScoreCalculator;
...
import com.cntest.fxpt.anlaysis.event.handler.XinjiangCalculateHandler;
 ...
 * @version 1.0
 */
public class StartAnalysisForXinjiang implements Runnable {
	private static final Logger log = LoggerFactory
			.getLogger(StartAnalysisForXinjiang.class);
	private ExamContext ec;

	private StartAnalysisForXinjiang(Exam exam) {
	...
	private void analysis() throws Exception {
		...
		disruptor
				.handleEventsWithWorkerPool(
						...
						new XinjiangFindCalcluateDataHander())
				.thenHandleEventsWithWorkerPool(new XinjiangCalculateHandler(),
						new XinjiangCalculateHandler())
	...
	private void loadExamData() throws Exception {
		...
		disruptor
				...
				.then(new CalculateCombinationSubjectScoreHandler(
						new XinjiangCombinationSubjectScoreCalculator()))
				.then(new EndHandler(countDownLatch));
	...
	public static void start(Exam exam) {
		StartAnalysisForXinjiang sa = new StartAnalysisForXinjiang(exam);
