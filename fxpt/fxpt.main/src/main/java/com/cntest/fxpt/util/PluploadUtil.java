...
public class PluploadUtil {  
     ...
     * @throws IOException 
     */  
    public static UploadFileInfo upload(Plupload plupload, File dir) throws IllegalStateException, IOException {  
        //生成唯一的文件名  
        String fileName = plupload.getName();//file.getOriginalFilename();
		String suffix = fileName.substring(fileName.lastIndexOf("."), fileName.length());
		String filename = DateUtils.getMillisecodeToString() + suffix;
        upload(plupload, dir, filename);  
		String filePath = dir +File.separator+ filename;
		UploadFileInfo  fileInfo=  new UploadFileInfo();
		fileInfo.setName(fileName).setDesFileName(filename) .setPath(filePath).setSuffix(suffix);
        return fileInfo;
    }  
