 ...
 * @version 1.0
 **/
public class SameNameSpecification extends AbstractSpecification<ExamStudent>{
    private static String sql = "SELECT COUNT(id) FROM kn_studentbase WHERE  NAME= ? ";
	@Override
	public boolean isInSatified(ExamStudent t) {
		JdbcTemplate jdbcTemplate = SpringContext.getBean("jdbcTemplate");
		int i = jdbcTemplate.queryForInt(sql, t.getName());
		return i > 0;
	}
