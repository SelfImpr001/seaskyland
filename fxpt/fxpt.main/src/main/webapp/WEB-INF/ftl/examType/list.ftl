...
<@breadcrumbs.breadcrumbs currentPath="${title!'考试类型设置'}">
<!-- 表单内容-->
  <#assign buttons=[{"text":"新增","id":"examType_create","class":"btn-primary","closeBtn":false}] >
  <@userContentTop.contentTop title="${title!'考试类型设置'}" buttons=buttons/>
  <#assign headers=["考试类型","是否有效","操作"]>
  <#assign rows=[]>
  <#assign buttons=[					
  ...
  <#assign optBtn=[]>
     <#if examTypes??>
	        	<#list examTypes as examType>
				  <#if examType.valid=true>
				     <#assign statusText="有效">
				  <#else>
				  	 <#assign statusText="无效">      
				  </#if>   
				  <#assign cells=[examType.name,statusText]>
				  <#assign rows=rows+[cells]>
				  <#assign optBtn=optBtn+[{"pk":examType.id,"buttons":buttons}]>
	        	</#list>
	        </#if>
  <@userGrid.grid headers=headers rows=rows buttons=optBtn pager=[] showPager=false hasFirstCheckbox=false showSeq=true idPreFix="examType" hasNested=true>
          ...
          <form class="form-horizontal " role="form">
			<div class="form-group  ">
			  ...
			  <div class="col-md-11 col-sm-11">
			    <input type="text" class="form-control" name="examTypeName" placeholder="请输入考试类型名称">
			  </div>	  
			</div>            
          </form>
        </div>
