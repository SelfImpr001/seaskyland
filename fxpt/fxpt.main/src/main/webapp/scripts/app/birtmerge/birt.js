(function(){
	...
	define(['jquery','ajax',"app/birt/mergeBirt",'formToJosn',"dialog","controller","app/birt/birtOrg", "ajaxfileupload","download",
	    ...
	    function bindEvent(container){
	    	...
	    	ctrl.initUI($obj); 
	    	$obj.find('#birtmerge_datagrid >div.query-form a.btn:eq(0)').on('click',function(){	    		
	    		self.query(getPager());
	    	});
	    	
	    	$obj.on('click','#birtmerge_datagrid tbody div.btn-group a',function(){
...
//    				
	    ...
	    function renderPager($pager) {	
	    	if($pager!=undefined){
	    		...
	    		ctrl.renderPager({
					"containerObj" : self.container,
					"pageObj" : self.container.find('#birtmerge_pager'),
					"callBack":self.query
				});	
	    	}
		}
...
//					},
	   ...
	   /**
		...
		function refreshTree(){
			...
			requestTreeData("birtmerge_ztree1", treeParentNode||treeNode);
		}
		var setting ;
		function ztree(){
		...
		function ajaxInitTree(level){
			ctrl.getJson("orgBirt/tree/list/"+level,function(data){
				...
				$zTree = $.fn.zTree.init(self.container.find("#birtmerge_ztree1"), setting, zNodes);
			});
		}
        ...
        function refreshs(htmlStr){
        	var $html = $(htmlStr);
        	var $tbody = self.container.find("#birtmerge_data_rows");
...
/**树相关*/
		function refresh(data){
			ctrl.refreshDataGrid('birtmerge_data_rows',self.container,$(data),renderPager);			
		};
		var page=ctrl.newPage()
		var _listUrl = "/birt/merge/list/" + page.curSize + "/" + page.pageSize+"?ui_all=true";
	    ...
	    var o = self = {
	       ...
	       setPage:function(index){
	    	   pages["curSize"] = index; 
	    	   _listUrl = "birt/merge/list/"+index+"/15?ui_all=true"; 
	       },
	       save:function(url){
...
//										}
           ...
           query:function(page){
        	   ...
        	   var url = "birt/merge/list/" + page.curSize + "/" + page.pageSize;
        	   var q = self.container.find('#birtmerge_datagrid >div.query-form input:text').val();
        	   if(q.length){
        		   url += "?q="+q;
        	   }
