(function() {
	...
	define(model, function($, ajax, ctrl, ajaxfileupload, download) {
		...
		function sUI(_opts) {
			...
			$.extend(opts, _opts);
			var url = "/upload/ui/" + opts.schemeType + "/ip?testPaperId=0";
		...
		function getFileUploadSetting() {
			...
			var url = window.app.rootPath + "upload/false/" + 111111 + "/"
					+ opts.schemeType;
			return {
				url : url,
				secureuri : false,
		...
		function getExistContentMessage() {
			var opts = $dialog.data("opts");
			var result = "导入的内容已存在，您确定要覆盖重新导入?";
			if(opts.importType==1){
				if (opts.schemeType === 1) {
					result = "您确定要追加导入?这样会保留其他学校学生信息，覆盖你当前导入的学校的学生信息！";
