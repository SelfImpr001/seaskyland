		...
		Parser, XMLReader // SAX1, SAX2
{
	...
	public void startDocument(XMLLocator locator, String encoding,
		...
		try {
			...
			if (fContentHandler != null) {
				if (locator != null) {
					fContentHandler
							.setDocumentLocator(new LocatorProxy(locator));
				}
	...
	public void startElement(QName element, XMLAttributes attributes,
		...
		try {
			...
			if (fDocumentHandler != null) {
				...
				fDocumentHandler
						.startElement(element.rawname, fAttributesProxy);
			}
	...
	public void attributeDecl(String elementName, String attributeName,
		...
		try {
			...
			if (fDeclHandler != null) {
				...
				String value = (defaultValue == null) ? null : defaultValue
						.toString();
		...
		try {
			...
			if (fDeclHandler != null) {
				...
				String systemId = fResolveDTDURIs ? identifier
						.getExpandedSystemId() : identifier
						.getLiteralSystemId();
	...
	public void parse(String systemId) throws SAXException, IOException {
		...
		// wrap XNI exceptions as SAX exceptions
			...
			if (ex == null || ex instanceof CharConversionException) {
				...
				throw (ex == null) ? new SAXParseException(e.getMessage(),
						locatorImpl) : new SAXParseException(e.getMessage(),
						locatorImpl, ex);
			}
	...
	public void setErrorHandler(ErrorHandler errorHandler) {
		...
		try {
			XMLErrorHandler xeh = (XMLErrorHandler) fConfiguration
		...
		try {
			...
			if (featureId.startsWith(Constants.SAX_FEATURE_PREFIX)) {
				...
				if (suffixLength == Constants.NAMESPACE_PREFIXES_FEATURE
						.length()
						&& featureId
								.endsWith(Constants.NAMESPACE_PREFIXES_FEATURE)) {
				...
				if (suffixLength == Constants.STRING_INTERNING_FEATURE.length()
						&& featureId
								.endsWith(Constants.STRING_INTERNING_FEATURE)) {
					if (!state) {
				...
				if (suffixLength == Constants.RESOLVE_DTD_URIS_FEATURE.length()
						&& featureId
								.endsWith(Constants.RESOLVE_DTD_URIS_FEATURE)) {
				...
				if (suffixLength == Constants.USE_ENTITY_RESOLVER2_FEATURE
						.length()
						&& featureId
								.endsWith(Constants.USE_ENTITY_RESOLVER2_FEATURE)) {
				...
				if ((suffixLength == Constants.IS_STANDALONE_FEATURE.length() && featureId
						.endsWith(Constants.IS_STANDALONE_FEATURE))
						|| (suffixLength == Constants.USE_ATTRIBUTES2_FEATURE
								.length() && featureId
								.endsWith(Constants.USE_ATTRIBUTES2_FEATURE))
						|| (suffixLength == Constants.USE_LOCATOR2_FEATURE
								.length() && featureId
								.endsWith(Constants.USE_LOCATOR2_FEATURE))
						|| (suffixLength == Constants.XML_11_FEATURE.length() && featureId
								.endsWith(Constants.XML_11_FEATURE))) {
					throw new SAXNotSupportedException(
							SAXMessageFormatter.formatMessage(
									fConfiguration.getLocale(),
									"feature-read-only",
			...
			/*
			 * else if (featureId.startsWith(XERCES_FEATURES_PREFIX)) { String
			 * feature = featureId.substring(XERCES_FEATURES_PREFIX.length());
			 * // // Drop through and perform default processing // }
			 */
	...
	public boolean getFeature(String featureId)
		...
		try {
			...
			if (featureId.startsWith(Constants.SAX_FEATURE_PREFIX)) {
				...
				// the org.xml.sax.ext.Locator2 interface.
				//
				if ((suffixLength == Constants.USE_ATTRIBUTES2_FEATURE.length() && featureId
						.endsWith(Constants.USE_ATTRIBUTES2_FEATURE))
						|| (suffixLength == Constants.USE_LOCATOR2_FEATURE
								.length() && featureId
								.endsWith(Constants.USE_LOCATOR2_FEATURE))) {
		...
		try {
			...
			if (propertyId.startsWith(Constants.SAX_PROPERTY_PREFIX)) {
				...
				if (suffixLength == Constants.LEXICAL_HANDLER_PROPERTY.length()
						&& propertyId
								.endsWith(Constants.LEXICAL_HANDLER_PROPERTY)) {
					try {
				...
				if (suffixLength == Constants.DECLARATION_HANDLER_PROPERTY
						.length()
						&& propertyId
								.endsWith(Constants.DECLARATION_HANDLER_PROPERTY)) {
					try {
				...
				// document.
				//
				if ((suffixLength == Constants.DOM_NODE_PROPERTY.length() && propertyId
						.endsWith(Constants.DOM_NODE_PROPERTY))
	...
	public ElementPSVI getElementPSVI() {
		return (fAugmentations != null) ? (ElementPSVI) fAugmentations
				.getItem(Constants.ELEMENT_PSVI) : null;
	}
	...
	public boolean interrupted() {
		...
		if (fContentHandler instanceof IInterrupte) {
			IInterrupte tmp = (IInterrupte) fContentHandler;
			isTrue = tmp.interrupted();
		}
		return isTrue;
	}
